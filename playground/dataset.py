...
def get_images_from_large_file(bands, year, region, stride):
    # local machine
    data_directory_path = '/home/tukl/Projects/Forest/Data/all_billion_tree_regions/landsat-8/train_data'
    destination = '/home/tukl/Projects/QasimWork/data/train/Pickled_data/'
...
def check_generated_pkl(path_to_pkl):
	with open(path_to_pkl, 'rb') as this_pickle:
	    print('log: Reading {}'.format(path_to_pkl))
	    (example_subset, label_subset) = pickle.load(this_pickle)
	show_image = np.asarray(255 * (example_subset[:, :, [4, 3, 2]] / 4096.0).clip(0, 1), dtype=np.uint8)
	pl.subplot(1,2,1)
	pl.imshow(show_image)
...
def get_dataloaders_generated_data(generated_data_path, data_split_lists_path, model_input_size, bands, num_classes, train_split, one_hot, batch_size,
                                   num_workers, transformation=None):
    ...
    class dataset(Dataset):
        def __init__(self, data_list, data_map_path, stride, mode='train', transformation=None):
            ...
            self.bands = [int(x)-1 for x in bands]
        ...
        def __getitem__(self, k):
            ...
            #print(np.unique(this_label_subset))
            #this_label_subset = fix(this_label_subset).astype(np.uint8)
            if self.one_hot:
                if this_label_subset.ndim == 3:
                    this_label_subset = this_label_subset.squeeze(-1)
                this_label_subset = np.eye(self.num_classes)[this_label_subset.astype(np.uint8)]
            if self.mode == 'train':
                # augmentation
                choice = np.random.randint(0, 3)
                # crop & rotate
                if choice == 0:
                    this_example_subset, this_label_subset = crop_and_rotate(image=this_example_subset, 
                                                                            label=this_label_subset,
                                                                            first_crop_size=128,
            ...
            # print(this_label_subset)
            # print(this_label_subset.shape)
    ...
    train_data = dataset(data_list=train_list, data_map_path=os.path.join(data_split_lists_path, 'train_datamap.pkl'), mode='train', stride=16,
...
def main():
    ...
    loaders = get_dataloaders_generated_data(generated_data_path='/home/tukl/Projects/QasimWork/data/train/Pickled_data/',
                                             data_split_lists_path="/home/tukl/Projects/QasimWork/data/train/training_lists",
                                             model_input_size=128, num_classes=2, bands=range(1,12), train_split=0.8, one_hot=True, 
...
if __name__ == '__main__':
    ...
    # all_districts = ["abbottabad", "battagram", "buner", "chitral", "hangu", 
                    # "haripur", "karak", "kohat", "kohistan", "lower_dir", 
                    # "malakand", "mansehra", "nowshehra", "shangla", "swat", 
                    # "tor_ghar", "upper_dir"
                    # ]
    ...
    #check_generated_pkl(f'/home/tukl/Projects/QasimWork/data/train/Pickled_data/abbottabad_2015_1.pkl')
    main()
