...
def mask_landsat8_image_using_rasterized_shapefile(rasterized_shapefiles_path, district, this_landsat8_bands_list):
    ...
    for idx, this_band in enumerate(this_landsat8_bands_list):
        ...
        this_band = adaptive_resize(this_band, shapefile_mask.T.shape)        
...
#             this_example_subset = this_example_subset[:, :, self.bands]
            this_example_subset = this_example_subset[:, :, here]
...
def run_inference(args,x,y):
    ...
    for district in all_districts:
        for year in years:
            ...
            test_image_path = os.path.join(args.data_path, 'landsat8_{}_region_{}.tif'.format(year, district))
...
def main():
    ...
    year_our = [2014, 2016]
    region_our = ['abbottabad']
    run_inference(args_better, year_our, region_our)
